...
from backend.Middlewares.Mail_Sender.sender import enviar_credenciales_gmail_antibiotico
...
def crear_empleado_zeus_antibiotico():
    ...
    try:
        ...
        if not retorno:
            return jsonify(msg=msj),400
        msj, retorno= enviar_credenciales_gmail_antibiotico(contacto[1],credenciales[0],credenciales[1])
        if not retorno:
            return jsonify(msg=msj),400
