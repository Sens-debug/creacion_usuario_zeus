...
from backend.Middlewares.Mail_Sender.sender import enviar_credenciales_gmail_cuidadora
...
def crear_empleado_zeus_cuidadora():
    ...
    print(contacto[0])
    try:
        msj, retorno=Conexion_Zeus.crear_personal_asistencial_cuidadora(primer_nombre=nombres[0],
        ...
        msj, retorno = Conexion_Zeus.crear_usuario_cuidadora(nombre_completo,numero_cedula,
        ...
        msj, retorno= enviar_credenciales_gmail_cuidadora(contacto[1],credenciales[0],credenciales[1])
        if not retorno:
            return jsonify(msg=msj),400
